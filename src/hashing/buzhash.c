...
const static uint32_t defaultBytehash[256] = {
...
Buzhash * buzhash_create(uint32_t blockSize, uint32_t *bytehash){
    Buzhash *bh = malloc(sizeof(Buzhash));
    if(bh == NULL)
        return NULL;
    
    bh->__blockSize = blockSize;
    bh->__buf = malloc(blockSize);
    if(bh->__buf == NULL){
        free(bh);
        return NULL;
    }
    ...
    bh->__blockSize = blockSize;
    bh->__bshift = blockSize % 32;
    
    if(bytehash != NULL)
        bh->__bytehash = bytehash;
    else
        bh->__bytehash = defaultBytehash; 
    buzhash_reset(bh);
    return bh;
}
...
static void buzhash_hash_byte(Buzhash *bh, uint8_t byte){
	if(bh->__bufpos == bh->__blockSize){
        bh->__overflow = true;
        bh->__bufpos = 0;
    }

    uint32_t hash = bh->__hash;
    hash = left_rot32(hash, 1);

    if(bh->__overflow){
        uint32_t toShift = bh->__bytehash[bh->__buf[bh->__bufpos]];
        hash ^= left_rot32(toShift, bh->__bshift);
    }
    bh->__buf[bh->__bufpos] = byte;
    bh->__bufpos++;

    hash ^= bh->__bytehash[byte];
    bh->__hash = hash;
}

void buzhash_write(Buzhash *bh, const uint8_t *input, size_t len){
    for(size_t i = 0; i < len; i++){
        buzhash_hash_byte(bh, input[i]);
    }
}