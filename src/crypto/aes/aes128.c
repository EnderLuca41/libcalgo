...
const static uint8_t S_Rijndael[256] = {
...
void rotword(uint8_t colunmn[4]){
    uint8_t buffer = colunmn[0];
    for(int i = 3; i != -1; i--){
        uint8_t buffer2 = colunmn[i];
        colunmn[i] = buffer;
        buffer = buffer2;
    }
}

static void key_schedule128(uint8_t initKey[16], uint8_t outputKeys[11][16]){
...
//Lookup tables for the mix columns step

const static uint8_t Mult2[256] = {
...
const static uint8_t Mult3[256] = {
...
const static uint8_t MixMatrix[16] = {
    2,3,1,1,
    1,2,3,1,
    1,1,2,3,
    3,1,1,2
...
static void mix_column(uint8_t b[4]){
    uint8_t output[4];

    for(int i = 0; i < 4; i++){
        uint8_t val = 0;
        for(int j = 0; j < 4; j++){
            switch (MixMatrix[i * 4 + j])
            {
            case 1:
                val ^= b[j];
                break;
...
static bool generate_block(uint8_t block[16], Aes128Buffer *buffer){
...
const static uint8_t S_RijndaelInverse[256] = {
...
//Lookup Tables for the inversion of mix columns

const static uint8_t Mult9[256] = {
...
const static uint8_t Mult11[256] = {
...
const static uint8_t Mult13[256] = {
...
const static uint8_t Mult14[256] = {
...
static void undo_mix_column(uint8_t b[4]){
    uint8_t output[4];

    for(int i = 0; i < 4; i++){
        uint8_t val = 0;
        for(int j = 0; j < 4; j++){
            switch (InverseMixMatrix[i * 4 + j])
            {
            case 9:
                val ^= Mult9[b[j]];
                break;
