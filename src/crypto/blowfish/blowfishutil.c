...
const static uint32_t S1Init[256] = {
...
const static uint32_t S2Init[256] = {
...
const static uint32_t S3Init[256] = {
...
void encrypt_block(uint32_t *l, uint32_t *r, const BlowfishContext *ctx){
    for(unsigned int i = 0; i < 16; i++){
        *l ^= ctx->subkeys[i];
        ...
        uint32_t newL = ctx->s1[*l >> 24];
        newL += ctx->s2[(*l >> 16) & 0xff];
        newL ^= ctx->s3[(*l >> 8) & 0xff];
        newL += ctx->s4[*l & 0xff];
...
void blowfish_init(const uint8_t *key, uint8_t keyLen, BlowfishContext *ctx){
    ...
    //Init S1-S4 and the Subkeys with the digits of pi in hexadecimal
    memcpy(ctx->subkeys, SubkeysInit, sizeof(SubkeysInit));
    memcpy(ctx->s1, S1Init, sizeof(ctx->s1));
    memcpy(ctx->s2, S2Init, sizeof(ctx->s2));
    memcpy(ctx->s3, S3Init, sizeof(ctx->s3));
    memcpy(ctx->s4, S4Init, sizeof(ctx->s4));

    //Xor key with subkeys
    unsigned int j = 0;
...
size_t generate_block_blowfish(uint32_t *l, uint32_t *r, const uint8_t *input, size_t inputSize){
    ...
    if(inputSize > 4){
        ...
        *r = 0;
        for(unsigned int i = 0; i < inputSize - 4; i++){
            *r |= input[i + 4] << (24 - i * 8);
        }
